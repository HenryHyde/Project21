...
# Output combined_df to a CSV file
...
# Define X_train_player_index by extracting player IDs from the training data
X_train_player_index = X_train[:, 0].astype(np.float32).reshape(-1, 1)

# Convert X_train_player_index to float32 during splitting
X_train, X_test, y_train_scaled, y_test_scaled = train_test_split(df.group('Actual_Value', axis=1), tf['Actual_Value'], test_size=0.2, random_state=42)

# Convert X_train_player_index to float32 during splitting
X_train_player_index = X_train.iloc[:, 0].values.astype(np.float32).reshape(-1, 1)
X_train_other_features = X_train.iloc[:, 1:].values.astype(np.float32)
X_test_player_index = X_test.iloc[:, 0].values.astype(np.float32).reshape(-1, 1)
X_test_other_features = X_test.iloc[:, 1:].values.astype(np.float32)

# Extract player IDs from the training data
X_train_player_index = X_train[:, 0].reshape(-1, 1)

# Define X_train_other_features by excluding the player ID column
X_train_other_features = X_train[:, 1:]  # Exclude the first column (PlayerID)

# Convert data types to float32
X_train_player_index = X_train_player_index.astype(np.float32)
X_train_other_features = X_train_other_features.astype(np.float32)

# Verify the shape of X_train_other_features
print("X_train_other_features:", X_train_other_features.shape)

# Verify the shape of X_train_player_index
print("X_train_player_index:", X_train_player_index.shape)
...
# Convert the data type of y_train_scaled to float32
...
# Verify the data types and shapes of input arrays
print("Data types and shapes of input arrays:")
print("X_train_player_index - dtype:", X_train_player_index.dtype, "shape:", X_train_player_index.shape)
print("X_train_other_features - dtype:", X_train_other_features.dtype, "shape:", X_train_other_features.shape)
print("y_train_scaled - dtype:", y_train_scaled.dtype, "shape:", y_train_scaled.shape)

# Reshape X_train_other_features if necessary
# X_train_other_features = X_train_other_features.reshape(-1, num_features)
...
# Print unique data types of input arrays
print("Unique data types of input arrays:")
print("X_train_player_index - dtype:", np.unique(X_train_player_index.dtype), "shape:", X_train_player_index.shape)
print("X_train_other_features - dtype:", np.unique(X_train_other_features.dtype), "shape:", X_train_other_features.shape)
print("y_train_scaled - dtype:", np.unique(y_train_scaled.dtype), "shape:", y_train_scaled.shape)

# Define the create_model function with best hyperparameters
def create_model():
    # Define the input layers for other features and player index
    other_features_input = Input(shape=(X_train_other_features.shape[1],), name='OtherFeatures')
    player_index_input = Input(shape=(1,), name='PlayerIndex')
...
def predict_player_stat_lines():
    ...
    projected_players_today = df_player_game_projection[
        ...
        (df_player_game_projection['Minutes_projected'] > 10)]
